...
void Buttons::update() {
  ...
  odroid_gamepad_state button_state;
  
  
  button_state = input_read_raw();


	if (button_state.values[ODROID_INPUT_DOWN]  == 1) buttonsData |= (1<<(int)Button::down);
	if (button_state.values[ODROID_INPUT_LEFT]  == 1) buttonsData |= (1<<(int)Button::left);
	if (button_state.values[ODROID_INPUT_RIGHT] == 1) buttonsData |= (1<<(int)Button::right);
	if (button_state.values[ODROID_INPUT_UP]    == 1) buttonsData |= (1<<(int)Button::up);

	if (button_state.values[ODROID_INPUT_B]     == 1) buttonsData |= (1<<(int)Button::a);
	if (button_state.values[ODROID_INPUT_A]     == 1) buttonsData |= (1<<(int)Button::b);
	if (button_state.values[ODROID_INPUT_SELECT]== 1) buttonsData |= (1<<(int)Button::menu);
	if (button_state.values[ODROID_INPUT_START] == 1) buttonsData |= (1<<(int)Button::home);

...
void loop() {
  ...
  //move the oponent
  if ((oponent_y + (oponent_h / 6)) < (ball_y + (ball_size / 6))) { //if the ball is below the oponent
  ...
  gb.display.setCursor(gb.display.width()/2 - gb.display.width()/4, gb.display.height()/2 - gb.display.height()/4);
